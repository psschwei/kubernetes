...
func DescribeProbe(probe *corev1.Probe) string {
	attrs := fmt.Sprintf("delay=%ds timeout=%ds period=%d period unit=%s #success=%d #failure=%d", probe.InitialDelaySeconds, probe.TimeoutSeconds, probe.PeriodSeconds, probe.ReadSecondsAs, probe.SuccessThreshold, probe.FailureThreshold)
	switch {
	case probe.Exec != nil:
